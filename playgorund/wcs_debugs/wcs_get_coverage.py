...
class WCSGetCoverageUser(HttpUser):
    ...
    def get_coverage(self):
        additional_params = config_obj["wcs"].RESOLUTIONS
        # or: additional_params={"scaleSize": config_obj["wcs"].SCALE_SIZE}
        # layer_xml = self.wcs_client.get_describe_coverage(self.coverage_id, self.session)
        subsets = self.wcs_client.generate_subset_from_xml(self.layer_xml,config_obj["wcs"].BBOX_WIDTH, config_obj["wcs"].BBOX_HEIGHT)

        res = self.wcs_client.get_coverage(self.coverage_id, config_obj["wcs"].FORMAT, subsets, additional_params,
