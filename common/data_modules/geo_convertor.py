...
def get_resolution_by_zoom_level(res_zoom_level: int) -> float:
    res = 180 / (pow(2, res_zoom_level)) / 256
    return res


def get_max_x_and_y_range(range_zoom_level, min_x, max_x, min_y, max_y):
    res = 180 / (pow(2, range_zoom_level)) / 256
...
zoom_level = 4
# minx_deg, miny_deg , max_x_deg , maxy_deg, zoom_level
# min_tile_x = floor((min_x_deg+180)/deg_per_tile,1)
# max_tile_x = floor((max_x_deg+180)/deg_per_tile,1)+1
