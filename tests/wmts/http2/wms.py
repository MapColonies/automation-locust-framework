...
def wmstileT(mylist):
    return f'api/raster/v1/service?LAYERS={config_obj["wms"].LAYER_TYPE}&FORMAT={config_obj["wms"].WMS_FORMAT}&SRS={config_obj["wms"].SRS}&TRANSPARENT=TRUE&service=wms&VERSION={config_obj["wms"].WMS_VERSION}&REQUEST=GetMap&STYLES=&BBOX={"{:.6f}".format(mylist[0])},{"{:.6f}".format(mylist[1])},{"{:.6f}".format(mylist[2])},{"{:.6f}".format(mylist[3])}&WIDTH={config_obj["wms"].WIDTH}&HEIGHT={config_obj["wms"].HEIGHT}&token={config_obj["wms"].TOK}'


def wmstileNoToken(mylist):
    return f'api/raster/v1/service?LAYERS={config_obj["wms"].LAYER_TYPE}&FORMAT={config_obj["wms"].WMS_FORMAT}&SRS={config_obj["wms"].SRS}&TRANSPARENT=TRUE&service=wms&VERSION={config_obj["wms"].WMS_VERSION}&REQUEST=GetMap&STYLES=&BBOX={"{:.6f}".format(mylist[0])},{"{:.6f}".format(mylist[1])},{"{:.6f}".format(mylist[2])},{"{:.6f}".format(mylist[3])}&WIDTH={config_obj["wms"].WIDTH}&HEIGHT={config_obj["wms"].HEIGHT}'
